...
const firstLetters = (arr) => {
  // Solution code here...
  let newArr = [];
  arr.map(i => {newArr.push(i.charAt(0));});
  return newArr;
};
...
const findHappiness = (arr) => {
  // Solution code here...
  let newArr = [];
  arr.map(i => {if (i.includes(':)')) {newArr.push(i); }});
  return newArr;
};
...
const standardizePhoneNumbers = (arr) => {
  ...
  arr.map(i => { let num = i.substring(1, 4) + i.substring(6, 9) + i.substring(10, 14);
    newArr.push(num);
  });
...
const allHappy = (arr) => {
  ...
  arr.map(i => {
    ...
    if (i.includes(':)')) {newArr.push(true);} 
    else {newArr.push(false);}

  });

  if (newArr.includes(false)) {boolean = false;}
   else {boolean = true;}
