...
const sortByChildren = (charArray) => {
  // Solution code here...
  charArray.sort((x, y) => {
    if (x.children.length > y.children.length) {return (1);}
     else if (x.children.length == y.children.length) {  let n = x.house.localeCompare(y.house);
           return (n);
    }
     else if (x.children.length < y.children.length) {return (-1);} 
    else { return (0);}
  });
