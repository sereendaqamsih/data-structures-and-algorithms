...
const screenForNames = (arr) => {
  ...
  let newArr = arr.filter(name => {
    if (/^((Mr||Mrs||Dr||Ms).\s)[A-Z]/.test(name)) {
      return name;
    }
  })
  return newArr;
...
const sortBy = (property, arr) => {
  // Solution code here...
  arr.sort((lowestPrice, highestPrice) => {
    if (lowestPrice[property] > highestPrice[property]) { return true; }
    else if (highestPrice[property] > lowestPrice[property]) { return -1; }
    else { return false }
  });
  return arr;
};
...
const isSecure = (url) => {
  // Solution code here...
  if (/^https:\/\//g.test(url)) { return true; }
  else { return false; }
};
